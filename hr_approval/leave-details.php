...
date_default_timezone_set('Asia/Kolkata');
$admremarkdate=date('Y-m-d G:i:s ', strtotime("now"));
$sql="update tblleaves set Hr_Remark=:description,Hr_Status=:status,Hr_RemarkDate=:admremarkdate where id=:did";
$query = $dbh->prepare($sql);
...
{         
  ...
  <tr>
                                            ...
                                            <td><?php echo htmlentities($result->LeaveType);?></td>
											 ...
											 <?php 
												$formatedFromDate = date("d F, Y", strtotime($result->FromDate));
												$formatedToDate = date("d F, Y", strtotime($result->ToDate));
											 ?>
                                            <td>From <?php echo htmlentities($formatedFromDate);?><br/>To <?php echo htmlentities($formatedToDate);?></td>
                                            <td style="font-size:16px;"><b>Posting Date</b></td>
											<?php 
												$formatedDate = date("d F, Y H:i:s", strtotime($result->PostingDate));
											?>
                                           <td><?php echo htmlentities($formatedDate);?></td>
                                        </tr>
...
<tr>
<td style="font-size:16px;"><b>Supervisor Remark: </b></td>
<td colspan="5"><?php
...
?></td>
 ...
 <tr>
<td style="font-size:16px;"><b>Supervisor Action taken date : </b></td>
<td colspan="5"><?php
if($result->AdminRemarkDate==""){
  echo "NA";  
}
else{
echo htmlentities(date("d F, Y H:i:s", strtotime($result->AdminRemarkDate)));
}
?></td>
 </tr>
 <tr>
 <td style="font-size:16px;"><b>HR Status :</b></td>
<td colspan="5"><?php $hrstats=$result->Hr_Status;
if($hrstats==1){
?>
<span style="color: green">Approved</span>
 <?php } if($hrstats==2)  { ?>
...
<?php } if($hrstats==0)  { ?>
 ...
 <tr>
<td style="font-size:16px;"><b>HR Remark: </b></td>
<td colspan="5"><?php
if($result->Hr_Remark==""){
  echo "waiting for Approval";  
}
else{
echo htmlentities($result->Hr_Remark);
}
?></td>
 </tr>

 <tr>
<td style="font-size:16px;"><b>HR Action taken date : </b></td>
<td colspan="5"><?php
if($result->Hr_RemarkDate==""){
  echo "NA";  
}
else{
echo htmlentities(date("d F, Y H:i:s", strtotime($result->Hr_RemarkDate)));
}
?></td>
 </tr>
 
<?php 
if($hrstats==0 && $stats==1)
{
